...
```
   ...
   a. The `-T` option will set the significance test for only CIF stack-heights. This can also be set to letters to calculate the significance for only CIF letter- heights. the default is for both.
   ```shell
   -T stacks 
   ```

   b. The `-P` option will set the number of permutations for significance test of CIFs to 100. 
   ```shell
   -P 100
   ```
...
```
   ...
   a. The `--kldperms` option will set the number of permutations to compute significance of Kullback-Leibler Divergences (KLD values) to 100
   ```shell
   --kldperms 100
   ```
...
```
   ...
   a. The `--idperms` will set the number of permutations to compute significance of Information Differences (ID values) to 100
   ```shell
   --idperms 100
   ```
   b. The `--clade` option will contrast two clades MAJOR and ENRIETTII against HOMO
   ```shell
   --clade HOMO
   ```
